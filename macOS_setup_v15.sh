...
if [[ "$USER_CHOICE_CREATE_ADMIN" =~ ^[Yy]$ ]]; then
    ...
    if [ -z "$NEW_ADMIN_NAME_INPUT" ]; then
        ...
        if dscl . -read "/Users/$NEW_ADMIN_SHORTNAME" > /dev/null 2>&1; then
            ...
            if [ -z "$LAST_UID" ] || [ "$LAST_UID" -lt 500 ]; then 
                NEW_UID=501
            else 
                NEW_UID=$((LAST_UID + 1))
            fi
            ...
            if [ $USER_CREATE_RESULT -eq 0 ]; then
                ...
                # ホームディレクトリ作成もエラーハンドリング
                set +e
                (cd / && sudo createhomedir -c -u "$NEW_ADMIN_SHORTNAME")
                HOME_CREATE_RESULT=$?
                set -e
                
                if [ $HOME_CREATE_RESULT -eq 0 ]; then
                ...
                # パスワード設定のエラーハンドリング
                set +e
                sudo dscl . -passwd "/Users/$NEW_ADMIN_SHORTNAME" "$NEW_USER_PASSWORD"
                PASSWORD_SET_RESULT=$?
                set -e
                
                if [ $PASSWORD_SET_RESULT -eq 0 ]; then
                    echo "  定義済みパスワードの設定に成功しました。"
                    echo "  管理者グループ (admin) に追加中..."
                    
                    # 管理者グループ追加のエラーハンドリング
                    set +e
                    sudo dscl . -append "/Groups/admin" GroupMembership "$NEW_ADMIN_SHORTNAME"
                    ADMIN_GROUP_RESULT=$?
                    set -e
                    
                    if [ $ADMIN_GROUP_RESULT -eq 0 ]; then
                        echo "  管理者グループ (admin) に追加しました。"
                        USER_CREATE_SUCCESS=true

                        # FileVaultに追加する (パスワードが「確定」状態のうちに)
                        if sudo fdesetup status | grep -q "FileVault is On."; then
                            echo "  FileVaultが有効なため、新しいユーザーをロック解除ユーザーとして追加します..."
                            
                            # FileVault追加のエラーハンドリング
                            set +e
                            sudo fdesetup add -usertoadd "$NEW_ADMIN_SHORTNAME" -user "$CURRENT_ADMIN_USER"
                            FILEVAULT_ADD_RESULT=$?
                            set -e
                            
                            if [ $FILEVAULT_ADD_RESULT -eq 0 ]; then
                        ...
                        # パスワード変更ポリシーは現在コメントアウト（FileVault追加との競合を避けるため）
                        ...
                        echo "⚠️ ユーザー「${NEW_ADMIN_SHORTNAME}」を管理者グループに追加できませんでした。"
                    fi
                else 
                    echo "⚠️ 定義済みパスワードの設定に失敗しました。"
                fi
            else 
                echo "⚠️ 基本ユーザーレコードの作成中にエラーが発生しました。"
            fi

            if [ "$USER_CREATE_SUCCESS" = true ]; then
                 echo "✅ 新しい管理者ユーザー「${NEW_ADMIN_SHORTNAME}」の基本的な作成プロセスが完了しました。"
            else
...
read -p "ログファイルをSMBサーバ (nas.aiming.local) にアップロードしますか？ [Y/n]: " UPLOAD_LOGS_SMB_INPUT
...
if [ "$PERFORM_SMB_UPLOAD" = true ] || [ "$PERFORM_SMB_DOWNLOAD" = true ]; then
    SMB_SERVER="nas.aiming.local"
    SMB_USER="aiming"
    DEFAULT_SMB_SHARE_NAME="INFRA-SETUP"
    ...
    if [ -z "$SMB_SHARE_NAME_TO_USE" ]; then 
        echo "⚠️ SMB共有名が指定されなかったため、SMB操作を中止します。" 
    else
        SMB_URL_FOR_CHECK="//${SMB_USER}@${SMB_SERVER}/${SMB_SHARE_NAME_TO_USE}"
        STANDARD_GUI_MOUNT_POINT="/Volumes/${SMB_SHARE_NAME_TO_USE}"
        FINAL_TARGET_BASE=""
        DO_MOUNT_UNMOUNT_BY_SCRIPT=true
        
        echo "SMB接続パスを準備中..."
        echo "  標準GUIマウントポイントを確認: ${STANDARD_GUI_MOUNT_POINT}"
        
        # 既存のマウントポイントをチェック
        if mount | grep -q -E "^${SMB_URL_FOR_CHECK} on ${STANDARD_GUI_MOUNT_POINT} \\(smbfs"; then
            if [ -d "${STANDARD_GUI_MOUNT_POINT}" ]; then 
                echo "✅ 標準GUIマウントポイント (${STANDARD_GUI_MOUNT_POINT}) が既に存在し、アクティブです。これを使用します。"
                FINAL_TARGET_BASE="${STANDARD_GUI_MOUNT_POINT}"
                DO_MOUNT_UNMOUNT_BY_SCRIPT=false
            else 
                echo "ℹ️ マウント情報に ${STANDARD_GUI_MOUNT_POINT} がありますが、ディレクトリとしてアクセスできません。一時マウントを試みます。"
            fi
        else 
            echo "ℹ️ 標準GUIマウントポイントは使用されていません、または目的の共有ではありません。一時マウントを試みます。"
        fi
        LOCAL_TEMP_MOUNT_POINT=""
        if [ "$DO_MOUNT_UNMOUNT_BY_SCRIPT" = true ]; then
            LOCAL_TEMP_MOUNT_POINT="/tmp/smb_ops_mount_$$_$(date +%s)"
            CAN_PROCEED_WITH_TEMP_MOUNT=true
            
            # 既存マウントポイントのチェック
            if mount | grep -q -F " on ${LOCAL_TEMP_MOUNT_POINT} "; then 
                echo "⚠️ 致命的エラー: 一時マウントポイントパス '${LOCAL_TEMP_MOUNT_POINT}' は既に他のファイルシステムとして使用されています。"
                CAN_PROCEED_WITH_TEMP_MOUNT=false
            else 
                mkdir -p "$LOCAL_TEMP_MOUNT_POINT"
        ...
        if [ -n "$FINAL_TARGET_BASE" ]; then
            if [ "$PERFORM_SMB_UPLOAD" = true ]; then
                ...
                DEFAULT_SMB_LOG_UPLOAD_PATH="macOS_filevault_key_backup"
                
                read -p "ログの保存先ディレクトリパス (共有 '${SMB_SHARE_NAME_TO_USE}' 内) を入力してください [$DEFAULT_SMB_LOG_UPLOAD_PATH]: " INPUT_SMB_LOG_UPLOAD_PATH
                SMB_LOG_UPLOAD_PATH_TO_USE=${INPUT_SMB_LOG_UPLOAD_PATH:-$DEFAULT_SMB_LOG_UPLOAD_PATH}
                SMB_LOG_UPLOAD_PATH_CLEAN=""
                
                # パスのクリーンアップ
                if [ -n "$SMB_LOG_UPLOAD_PATH_TO_USE" ]; then 
                    SMB_LOG_UPLOAD_PATH_CLEAN=$(echo "$SMB_LOG_UPLOAD_PATH_TO_USE" | sed 's#^/*##' | sed 's#/*$##')
                fi
                
                # アップロード先ディレクトリの決定
                FINAL_LOG_UPLOAD_DIR_ON_SMB="$FINAL_TARGET_BASE"
                if [ -n "$SMB_LOG_UPLOAD_PATH_CLEAN" ]; then 
                    FINAL_LOG_UPLOAD_DIR_ON_SMB="${FINAL_TARGET_BASE}/${SMB_LOG_UPLOAD_PATH_CLEAN}"
                fi
                
                echo "ログアップロード先ディレクトリ: ${FINAL_LOG_UPLOAD_DIR_ON_SMB}"
                echo "ディレクトリを作成 (または確認) 中..."
                mkdir -p "${FINAL_LOG_UPLOAD_DIR_ON_SMB}"
                
                if [ ! -d "${FINAL_LOG_UPLOAD_DIR_ON_SMB}" ]; then 
                    echo "⚠️ ログアップロード先ディレクトリの作成/確認に失敗しました: ${FINAL_LOG_UPLOAD_DIR_ON_SMB}"
                else
                    # 完全セッションログのアップロード
                    if [ -f "$FULL_SESSION_LOG_FILE" ]; then 
                        echo "rsync を使用して完全セッションログ '$FULL_SESSION_LOG_FILE' をアップロード中 (進捗表示あり)..."
                        
                        set +e
                        rsync -ah --progress "$FULL_SESSION_LOG_FILE" "${FINAL_LOG_UPLOAD_DIR_ON_SMB}/"
                        UPLOAD_RESULT=$?
                        set -e
                        
                        if [ $UPLOAD_RESULT -eq 0 ]; then 
                            echo "✅ 完全セッションログのアップロードに成功しました。"
                        else 
                            echo "⚠️ 完全セッションログのアップロードに失敗しました。"
                        fi
                    else 
                        echo "⚠️ 完全セッションログファイル '$FULL_SESSION_LOG_FILE' が見つからないため、アップロードをスキップします。"
                    fi
                    echo ""
                    
                    # 復旧キーログのアップロード
                    if [ -f "$RECOVERY_KEY_ONLY_LOG_FILE" ]; then 
                        echo "rsync を使用して復旧キーログ '$RECOVERY_KEY_ONLY_LOG_FILE' をアップロード中 (進捗表示あり)..."
                        
                        set +e
                        rsync -ah --progress "$RECOVERY_KEY_ONLY_LOG_FILE" "${FINAL_LOG_UPLOAD_DIR_ON_SMB}/"
                        UPLOAD_RESULT=$?
                        set -e
                        
                        if [ $UPLOAD_RESULT -eq 0 ]; then 
                            echo "✅ 復旧キーログのアップロードに成功しました。"
                        else 
                            echo "⚠️ 復旧キーログのアップロードに失敗しました。"
                        fi
                    else 
                        echo "⚠️ 復旧キーログファイル '$RECOVERY_KEY_ONLY_LOG_FILE' が見つからないため、アップロードをスキップします。"
                    fi
                fi
            ...
            if [ "$PERFORM_SMB_DOWNLOAD" = true ]; then
                ...
                DOWNLOAD_ITEMS=(
                    ...
                    # 新しいアイテムを追加する場合は、以下の形式で追記してください：
                    # "表示名|SMB共有上のパス|説明"
                )
                ...
                if [ -n "$LOGGED_IN_USER" ] && [ "$LOGGED_IN_USER" != "root" ]; then 
                    ...
                    if [ -d "$USER_HOME_DIR/Desktop" ]; then 
                        USER_DESKTOP_PATH="$USER_HOME_DIR/Desktop"
                    fi
                fi
                
                # フォールバック先の設定
                if [ -z "$USER_DESKTOP_PATH" ] || ! [ -d "$USER_DESKTOP_PATH" ]; then 
                    USER_DESKTOP_PATH="$HOME/Desktop"
                fi
                
                if [ ! -d "$USER_DESKTOP_PATH" ]; then 
                    echo "⚠️ ダウンロード先デスクトップフォルダが見つかりません。ダウンロード処理を中止します。"
                else
                    echo "ダウンロード先デスクトップ: $USER_DESKTOP_PATH"
                    echo ""
                    ...
                    if [ ${#DOWNLOAD_CHOICES[@]} -eq 0 ]; then
                        echo "すべてのアイテムがスキップされたため、ダウンロードは実行されませんでした。"
                    else
                        echo "選択されたアイテムをダウンロードします..."
                        echo ""
                        
                        for selected_item in "${DOWNLOAD_CHOICES[@]}"; do
                            DOWNLOADS_ATTEMPTED=$((DOWNLOADS_ATTEMPTED + 1))
                            
                            # アイテム情報の解析
                            IFS='|' read -r DISPLAY_NAME SMB_PATH DESCRIPTION <<< "$selected_item"
                            FILE_NAME=$(basename "$SMB_PATH")
                            SOURCE_FULL_PATH="${FINAL_TARGET_BASE}/${SMB_PATH}"
                            ...
                            if [ -e "$SOURCE_FULL_PATH" ]; then 
                                set +e
                                rsync -ah --progress "$SOURCE_FULL_PATH" "$USER_DESKTOP_PATH/"
                                COPY_RESULT=$?
                                set -e
                                
                                if [ $COPY_RESULT -eq 0 ]; then 
                                    echo "✅ 「${DISPLAY_NAME}」のダウンロードに成功しました。"
                                    DOWNLOADS_SUCCEEDED=$((DOWNLOADS_SUCCEEDED + 1))
                                else 
            ...
            if [ "$DO_MOUNT_UNMOUNT_BY_SCRIPT" = true ] && [ -n "$LOCAL_TEMP_MOUNT_POINT" ]; then
                if [ ! -d "$LOCAL_TEMP_MOUNT_POINT" ]; then 
                    echo "ℹ️ 一時マウントポイントディレクトリ ${LOCAL_TEMP_MOUNT_POINT} が存在しません。アンマウント処理は不要または実行不可能です。"
                else
                    echo ""
                    ...
                    # 通常のアンマウント試行
                    set +e
                    UNMOUNT_OUTPUT_NORMAL=$(diskutil unmount "$LOCAL_TEMP_MOUNT_POINT" 2>&1)
                    UNMOUNT_STATUS_NORMAL=$?
                    set -e
                    
                    if [ $UNMOUNT_STATUS_NORMAL -eq 0 ]; then 
                        echo "✅ 一時SMB共有のアンマウントに成功しました (通常)。"
                        sleep 1
                    else
                        if echo "$UNMOUNT_OUTPUT_NORMAL" | grep -q -i "not currently mounted"; then 
                            echo "ℹ️ 一時SMB共有は既にアンマウントされているようです (通常試行時)。"
                        else
                            echo "⚠️ 通常のアンマウントに失敗しました。エラー: $UNMOUNT_OUTPUT_NORMAL"
                            echo "   強制アンマウントを試みます..."
                            
                            # 強制アンマウント試行
                            set +e
                            UNMOUNT_OUTPUT_FORCE=$(diskutil unmount force "$LOCAL_TEMP_MOUNT_POINT" 2>&1)
                            UNMOUNT_STATUS_FORCE=$?
                            set -e
                            
                            if [ $UNMOUNT_STATUS_FORCE -eq 0 ]; then 
                                echo "✅ 一時SMB共有のアンマウントに成功しました (強制)。"
                                sleep 1
                            elif echo "$UNMOUNT_OUTPUT_FORCE" | grep -q -i "not currently mounted"; then 
                                echo "ℹ️ 一時SMB共有は既にアンマウントされているようです (強制試行後)。"
                            else 
                                echo "⚠️ 強制アンマウントにも失敗しました。エラー: $UNMOUNT_OUTPUT_FORCE"
                                echo "   手動でアンマウントする必要があるかもしれません: $LOCAL_TEMP_MOUNT_POINT"
                    ...
                    rmdir "$LOCAL_TEMP_MOUNT_POINT" >/dev/null 2>&1
                    if [ -d "$LOCAL_TEMP_MOUNT_POINT" ]; then 
                        echo "ℹ️ 一時マウントポイントディレクトリ $LOCAL_TEMP_MOUNT_POINT は削除できませんでした。手動での確認が必要な場合があります。"
                    else 
                        echo "✅ 一時マウントポイントディレクトリ $LOCAL_TEMP_MOUNT_POINT を削除しました。"
                    fi
                fi
            elif [ "$DO_MOUNT_UNMOUNT_BY_SCRIPT" = false ] && [ -n "$FINAL_TARGET_BASE" ]; then 
                echo ""
                echo "ℹ️ 既存のマウントポイント (${FINAL_TARGET_BASE}) を使用したので、スクリプトはアンマウントを行いませんでした。"
            fi
