                ...
                if sudo dscl . -passwd "/Users/$NEW_ADMIN_SHORTNAME" "$NEW_USER_PASSWORD"; then
                    ...
                    if sudo dscl . -append "/Groups/admin" GroupMembership "$NEW_ADMIN_SHORTNAME"; then
                        ...
                        if sudo fdesetup status | grep -q "FileVault is On."; then
                            ...
                            echo ""
                            read -sp "  FileVault認証のため、現在の管理者 ($CURRENT_ADMIN_USER) のパスワードを入力してください: " CURRENT_ADMIN_PASSWORD
                            echo ""

                            if sudo fdesetup add -usertoadd "$NEW_ADMIN_SHORTNAME" -inputstdin <<< "$(printf "%s\n%s" "$NEW_USER_PASSWORD" "$CURRENT_ADMIN_PASSWORD")" > /dev/null 2>&1; then
