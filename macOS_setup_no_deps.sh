...
enable_filevault() {
    ...
    if [[ "$REPLY" == "" ]]; then
        echo ""
        echo "🚀 FileVault の有効化を試みます（外部依存なし）。"
        echo "完全なセッションは一時的に '$TMP_FULL_LOG_FILE' に記録しています..."
        echo ""
        
        # FileVault有効化実行（外部依存なしバージョン）
        local filevault_result=0
        if [[ "$USE_STORED_PASSWORD" == true ]]; then
            # 自動化モード：標準入力でパスワードを渡す
            echo "🤖 自動化モードでFileVaultを有効化します..."
            printf '%s\n' "$STORED_ADMIN_PASSWORD" | sudo -S fdesetup enable -u "$current_admin_user" 2>&1 | tee "$TMP_FULL_LOG_FILE"
            filevault_result=$?
        else
            # 手動モード：scriptを使用
...
# メイン実行関数
main() {
    echo "🚀 macOS セットアップスクリプト v15 (外部依存なし版) を開始します..."
    echo ""
